...
charSet = {
    "G" : [ 0x7E, 0x80, 0x80, 0x80, 0x82, 0x82, 0x82, 0x7E ]
}

def drawChar(x, y, c, colour):
    if True:#c in charSet:
        # set column start/end
        LEFT_HI = (x >> 8) & 255
        LEFT_LO = x & 255
        ...
        for bitmapRow in charSet[c]:
            mask = 128
            for _ in range(0,8):
                if bitmapRow & mask != 0:
                    pixelData += [COLOUR_HI, COLOUR_LO]
                else:
                    pixelData += [0, 0]
                mask >>= 1

        sendDisplayCommand(0x2C, pixelData)

fillRect(10, 10, 17, 17, 0x001f)
drawChar(10, 10, 'G', 0x07C0)
drawChar(18, 10, 'G', 0xf800)
