...
def fillRect(left, top, right, bottom, colour):
    ...
    width = right-left+1
    rowBuffer = bytearray(2*width)
    for index in range(0, 2*width, 2):
        rowBuffer[index] = COLOUR_HI
        rowBuffer[index+1] = COLOUR_LO
    PIN_RS.write_digital(1)
...
fillRect(x, y, x+size-1, y+size-1, 0xffff)
...
while True:
    ...
    # a brute-force fill results in a very flickery square
#    fillRect(x, y, x+size, y+size, 0xffff)

    # instead, just update the pixels that have changed!
    if x < oldX:
        if y < oldY:
            # draw new top-left
            fillRect(x, y, oldX-1, y+size-1, 0xffff)
            fillRect(oldX, y, x+size-1, oldY-1, 0xffff)

            # clear old bottom-right
            fillRect(oldX, y+size, oldX+size-1, oldY+size-1, 0x0)
            fillRect(x+size, oldY, oldX+size-1, y+size-1, 0x0)

        elif y > oldY:
            # draw new bottom-left
            fillRect(x, y, oldX-1, y+size-1, 0xffff)
            fillRect(oldX, oldY+size, x+size-1, y+size-1, 0xffff)

            # clear old top-right
            fillRect(oldX, oldY, oldX+size-1, y-1, 0x0)
            fillRect(x+size, y, oldX+size-1, oldY+size-1, 0x0)
            ...
            # clear old right
            fillRect(x+size, y, oldX+size-1, y+size-1, 0x0)

    elif x > oldX:
        if y < oldY:
            # draw new top-right
            fillRect(x, y, x+size-1, oldY-1, 0xffff)
            fillRect(oldX+size, oldY, x+size-1, y+size-1, 0xffff)

            # clear old bottom-left
            fillRect(oldX, y+size, oldX+size-1, oldY+size-1, 0x0)
            fillRect(oldX, oldY, x-1, y+size-1, 0x0)
        elif y > oldY:
            # draw new bottom-right
            fillRect(x, oldY+size, x+size-1, y+size-1, 0xffff)
            fillRect(oldX+size, y, x+size-1, oldY+size-1, 0xffff)

            # clear old top-left
            fillRect(oldX, oldY, oldX+size-1, y-1, 0x0)
            fillRect(oldX, y, x-1, oldY+size-1, 0x0)
        else:
            # draw new right
            fillRect(oldX+size, y, x+size-1, y+size-1, 0xffff)

            # clear old left
            fillRect(oldX, y, x-1, y+size-1, 0x0)
    else:
        if y < oldY:
            # draw new top
            fillRect(x, y, x+size-1, oldY-1, 0xffff)
            ...
            # clear old bottom
            fillRect(x, y+size, x+size-1, oldY+size-1, 0x0)
            pass
        elif y > oldY:
            # draw new bottom
            fillRect(x, oldY+size, x+size-1, y+size-1, 0xffff)
            ...
            # clear old top
            fillRect(x, oldY, x+size-1, y-1, 0x0)
        else:
            # it didn't move?!
            pass
